...
function init() {
	...
	var City = function City(width, depth, quadrant) {
		this.width = width;
		this.depth = depth;
		this.scene = new THREE.Scene();
		this.entities = {};
		this.landscape = new Landscape(this.width, this.depth);
	...
	// size/blockSize must be odd for even building placement
	...
	var city = new City(340, 340, 16);
	var sky = new Sky(400);
	sky.surround(city);
	city.add(sky);
	//city.generateGrid();
	var borough = new Borough(305, 12, 10, 30, 12);
	...
	function renderScene() {
		...
		camera.lookAt(new THREE.Vector3(city.width / 2, 0, city.depth / 2));
		renderer.render(city.scene, camera);
