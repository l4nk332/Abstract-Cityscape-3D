...
function init() {
	...
	City.prototype.generateGrid = function () {
		for (var i = 0; i < this.numberOfBoroughs; i++) {
			for (var j = 0; j < this.numberOfBoroughs; j++) {
				...
				var blockSize = boroughSize/13;
				var borough = new Borough(boroughSize, blockSize, blockSize/3, blockSize*4, blockSize/3);
	...
	var city = new City(1000, 3);
	var sky = new Sky(1050);
	sky.surround(city);
	city.add(sky);
	city.generateGrid();
